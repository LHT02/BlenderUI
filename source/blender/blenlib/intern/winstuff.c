...
/* BLUI: icon extraction used to live here. It moved to
 * `intern/ghost/intern/GHOST_ShellMenuWin32.cc` for two reasons: it needs
 * `IImageList`, which is a COM interface and therefore not usable from this C
 * translation unit, and the GHOST SDK-only file is the one place it can be
 * tested without a window - see `blui/tools/shellmenu_selftest.cc`. The file
 * browser now calls `GHOST_LoadFileIconRgba()`.
 *
 * `SHGFI_ICON` was the other loss: it only ever returns the small system icon,
 * where going through the image list gets one at a usable size. */
