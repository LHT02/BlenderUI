...
def km_window(params):
    ...
    items.extend([
        ...
        #
        # BLUI keeps this shortcut row - it is a genuinely useful way to move an
        # area between components - but the list is BLUI's component set.
        # Blender's F2 (Movie Clip), F3 (Node Editor), F5 (3D Viewport), F6
        # (Graph Editor), F7 (Properties), F9 (Outliner) and F12 (Dope Sheet)
        # name editors this product does not register, and `space_type` is
        # validated when the keymap is created, so leaving them in is an error
        # at startup rather than a shortcut that quietly does nothing.
        *(
